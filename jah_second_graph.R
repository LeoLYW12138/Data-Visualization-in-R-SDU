...
choices_y = c("Annual.Growth.Rate..","Rate.of.Natural.Increase","Population.Density..People.per.Sq..Km..","Natural.Increase",                       
    "Total.Fertility.Rate","Crude.Birth.Rate","Gross.Reproduction.Rate",                
    "LifeExp","Infant.Mortality.Rate..Both.Sexes","Crude.Death.Rate","Deaths..both.sexes","Net.Migration.Rate","Net.international.migrants..both.sexes",
...
continent = c("Europe","South America","Asia","North America","Africa")

# Define the UI
ui <- fluidPage(
  titlePanel("Are there any changes in each variable when a country shifts from one income group to another?"),
  sidebarLayout(
    sidebarPanel(
      selectInput("continent", "Select Continent", choices = continent, multiple = TRUE, selected = "Asia"),
      selectInput("yaxis", "Select Y-axis", choices = choices_y, selected = choices_y[1])
    ),
    mainPanel(
...
server <- function(input, output) {
  ...
  dff <- dff %>%
    mutate(Year = as.integer(Year),
           ...
           Annual.Growth.Rate.. = as.numeric(Annual.Growth.Rate..),
           Rate.of.Natural.Increase = as.numeric(Rate.of.Natural.Increase),
           Population.Density..People.per.Sq..Km.. = as.numeric(Population.Density..People.per.Sq..Km..),
